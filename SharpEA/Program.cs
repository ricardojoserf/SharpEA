    ...
    internal class Program
    {
        ...
        static int readvals(byte[] data, int offset) {
            ...
            for (int i = (offset + ea_name_length + 1); i < (offset + ea_name_length + 1 + ea_value_length); i = i + 8)
            {
                ...
                Console.WriteLine("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t\t{8}", data[i], data[i + 1], data[i + 2], data[i + 3], data[i + 4], data[i + 5], data[i + 6], data[i + 7], value_str);
            }
            return next_entry_int;
        }


        static void ReadEA(String ea_filename) {
            ...
            int name_length = ffeai.EaNameLength;
            int value_length = ffeai.EaValueLength;
            ...
            Console.WriteLine("[+] EaNameLength:                {0}  \t(0x{1})", name_length, name_length.ToString("x"));
            Console.WriteLine("[+] EaValueLength:               {0}  \t(0x{1})", value_length, value_length.ToString("x"));
            ...
            */
            ...
            // EA Name
            byte[] name_bytes = ea_content_arr.Take(name_length).ToArray();
            var name_str = System.Text.Encoding.Default.GetString(name_bytes);
            Console.WriteLine("[+] EA Name: " + name_str);

            // EA Value
            Console.WriteLine("[+] EA Value: ");
            for (int i = (name_length + 1); i < (name_length + 1 + value_length); i = i + 8)
            {
                String value_str = "";
                for (int j = 0; j < 8; j++)
                {
            ...
            while (next_entry != 0) {
                next_entry = readvals(ea_content_arr, next_entry);
            }
